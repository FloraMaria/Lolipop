...
# Prints the top bar of search results.
def TopBar(pg):
	print "Page: " + str(pg)
...
def FetchResults(searchTerm, page_no):
	...
	TopBar(page_no)
	# Requests are sent to nyaa.se ...
	search_term_array = searchTerm.split()
	search_site = "http://www.nyaa.se/?page=search&cats=0_0&filter=0&term="
	...
	# And lxml easily sorts through the data!
...
# Pages are too big to properly fit in one terminal. This takes care of the half.
...
def viewFile(titles, seeders_no, download_links, types, sizes, num, links):
	...
	while True:
		...
		if choice == 'y' or choice == 'Y':
			torrentmodules.Torrent(num, titles, download_links)
